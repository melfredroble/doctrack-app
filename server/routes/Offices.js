...
router.get("/", requireAuth, (req, res) => {
  conn.query("SELECT  * FROM `offices`", (error, result) => {
    if (result) {
...
//   conn.query("SELECT  office_name FROM `offices`", (error, result) => {
//     if (result) {
//       result;
//       res.status(201).json({ result );
//       console.log({ ...result });
...
router.get("/:id", requireAuth, (req, res) => {
  ...
  conn.query("SELECT * FROM `offices` WHERE id = ?", id, (error, result) => {
    if (result) {
      res.json(result);
    }
    if (error) {
