...
router.get("/:id", requireAuth, (req, res) => {
  ...
  conn.query(
    "SELECT users.id, users.name, users.email, users.password, users.office_id, users.role FROM users JOIN offices ON users.office_id = offices.id WHERE users.id = ?",
    userId,
    (err, result) => {
      if (result) {
        res.status(200).json(result);
  ...
  if (hashedPassword !== "") {
    bcrypt.hash(hashedPassword, saltRounds, (error, hash) => {
      if (error) {
        ...
        conn.query(
          "UPDATE `users` SET `name`= ?,`email`= ?,`password`= ?,`office_id`= ?,`role`= ? WHERE id = ?",
          [fullName, email, hash, office, role, id],
          (error) => {
            if (error) {
              console.log(error);
            } else {
    ...
    conn.query(
      "UPDATE `users` SET `name`= ?,`email`= ?,`office_id`= ?,`role`= ? WHERE id = ?",
      [fullName, email, office, role, id],
      (error) => {
        if (error) {
          console.log(error);
        } else {
